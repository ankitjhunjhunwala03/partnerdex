...
| **MRR contribution by app** | MRR as of the end of the range, split by the app earning it, with each app's share of the total. Every app gets its own row, ordered largest first. Metered usage is included wherever the MRR card includes it, so the parts sum to the MRR headline by construction. |
| **MRR contribution by plan** | MRR as of the end of the range, split by the plan each subscription is on, with each plan's share of the total. The plan is the charge name Shopify recorded, so the rows are keyed on app *and* plan — two apps both selling a "BASIC" stay two plans, labelled by app whenever more than one is in scope. Annual plans contribute a twelfth of their price, as everywhere else. Metered usage carries no charge of its own, so it is credited to the plan its shop was on at the end of each bucket — the same shop-and-app attribution churn uses — and consumption by a shop with no live subscription gets its own row rather than being folded into a plan that did not earn it. The parts therefore sum to the **MRR** headline under whatever component filters are set. |
| **Subscriptions by plan** | The same split, counting live contracts instead of money. Subscriptions rather than subscribers: one shop can hold charges on two plans, and counting merchants would produce rows adding to more than the subscriber headline. The parts sum to **Active subscriptions** exactly. |
...
| **Trials** | Count of trials started in the period, split into converted and cancelled. A trial resolves to `converted` (a payment landed), `canceled` (ended before any payment), `in_trial` (still inside the free window), `awaiting_usage` (a usage-priced plan whose free window closed with nothing consumed — nothing forces a decision on a metered plan, so the outcome is still open and stays outside the conversion ratio) or `unknown` (a data gap). |
| **On trial** | Instantaneous count of active trials at that exact point in time, on priced and usage-priced plans alike. A free window is only recognised when the first billing date sits *less* than a full billing cycle after activation: a first bill a whole cycle out is the plan's own rhythm, not a trial. A trial as long as the cycle it precedes (a 30-day trial on 30-day billing) is therefore indistinguishable from an ordinary cycle while it is still in flight, and is only labelled once it resolves. |
| **Trialing** | Current trial subscription value grouped by each expected trial end date. A usage-priced plan has no contracted price to forecast, so it is valued at the mean amount its plan's settled shops billed over the last 30 days — shops that consumed nothing included, shops still trialling excluded. That figure is an estimate, not a booked amount, and `meta.estimatedTrials` and `meta.estimateSample` report how much of the forecast rests on it. |
...
1. **Inferred Trials:** Trial periods are detected based on the gap between subscription activation and the merchant's first payment. `TRIAL_MIN_GAP_DAYS` (default `2`) defines this threshold. What counts as the first payment depends on how the plan bills: a priced plan is paid by an `AppSubscriptionSale` on its own charge, while a **usage-priced plan** — recurring amount of zero, billed entirely through metered usage — is paid by the shop's first `AppUsageSale` after that charge activated. A usage plan never raises a subscription sale, so keying the inference on price alone made every merchant on one invisible to every trial report.
...
3. **Usage Revenue Recognition:** A metered usage charge is recognized as a monthly rate across the term it paid for, not summed inside a fixed window. Ordinary consumption is recognized at its full amount for 30 days — that *is* the month's spend. A charge that bought a *year* is recognized at a twelfth of itself for 365 days, the same normalization an annual subscription price gets. That applies where the shop's plan bills annually **and** carries a recurring price of zero, meaning the usage charge is how the plan is paid at all; metered spend on top of a plan that already has a price stays in the month it was consumed. Because a zero-priced plan gives Shopify's feeds nothing to state an interval with, the annual case is recognised only where `ANNUAL_PLAN_PATTERN` matches the plan's name — see `.env.example`. Without it, a year collected in one charge reads as one month of run rate and then vanishes.
4. **Store deactivation vs. uninstall:** an uninstall is the merchant removing the app and ends the subscription (`churn_reason = uninstalled`). A *deactivation* is the shop itself pausing, closing or being suspended, and Shopify answers it by **freezing** the charge rather than cancelling it — so where a freeze follows, the subscription reads as `subscription_frozen` and is not churned at all, because the store can reopen. A deactivation with no freeze does end the subscription, under `churn_reason = deactivated`, kept apart from `uninstalled` so a churn-reason breakdown is not polluted by store closures the app had no part in.
5. **Plan Upgrades/Downgrades:** Shopify models plan changes by cancelling the old subscription and creating a new one, in a single operation. PartnerDex pairs a cancel with an activation that lands within 60 seconds of it — forward only, since an activation cannot replace a charge cancelled after it — so an upgrade is not reported as churn. A merchant who cancels and comes back later is a cancellation followed by a win-back (`resubscribed`), however soon they return.
