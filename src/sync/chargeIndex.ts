 ...
 *
 * This is the payment signal for a subscription that has no price of its own. A
 * usage-priced plan carries a recurring amount of zero and therefore never
 * produces an `AppSubscriptionSale`, so the question the trial inference is
 * built on — "has this merchant paid yet?" — has no answer in the subscription
 * feed at all. It has one here.
 *
 * Keyed by shop-and-app rather than by charge because usage sales carry an
 * `AppUsageRecord` id and never the subscription they belong to; this is the
 * same attribution the churn and MRR-by-plan reports make, for the same reason.
 *
 * Sliced like the loaders above rather than read whole: `idx_tx_shop_time` is
 * `(app_id, shop_id, created_at)`, so each pair is a range seek that arrives
 * already ordered, which is what lets `usagePaidAfter` stop at the first match.
 */
export function usageForPairs(db: Db): Map<string, string[]> {
  const rows = db
    .prepare(
      `SELECT t.app_id, t.shop_id, t.created_at
         FROM temp.derive_slice p
         CROSS JOIN transactions t ON t.app_id = p.app_id AND t.shop_id = p.shop_id
        WHERE t.type = 'AppUsageSale' AND t.gross_amount > 0
        ORDER BY t.app_id, t.shop_id, t.created_at`,
    )
    .all() as Array<{ app_id: string; shop_id: string; created_at: string }>;
  ...
  for (const row of rows) {
    ...
    else ledger.set(key, [row.created_at]);
  }
  return ledger;
}
